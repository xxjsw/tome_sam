...
def pitome(
    ...
    B, T, T = sim.shape
    # separate protected token and mergable tokens
    ...
    sim = gather(sim, dim=-1, index=b_idx.unsqueeze(-2).expand(B, T, r))
    sim = gather(sim, dim=-2, index=a_idx.unsqueeze(-1).expand(B, r, r))

    _, dst_idx = sim.max(dim=-1) # dst_idx (B, r) for each src token records the index of the dst token
    ...
    def merge(x: torch.Tensor, mode="mean") -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        dst = dst.scatter_reduce(dim=1, index=dst_idx.unsqueeze(-1).expand(B, r, C), src=src, reduce=mode)
...
def pitome_vision(
    ...
    with torch.no_grad():
        ...
        if use_bsm_pitome:
            ...
            return pitome(metric=metric, indices=indices, sim=sim, r=r)
